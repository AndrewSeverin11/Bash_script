...
if [[ $u_indx = 999 ]]
then
echo -e "\n\n\033[42;30;1m Скачивается образ диска обновлений \033[0m\n"
u_indx=3
path_update=${path_iso[$u_indx]}
cd $path_update
wget --no-check-certificate https://dl.astralinux.ru/astra/stable/smolensk/security-updates/1.6/$iso_update/$iso_update$iso_ext
sudo rm *.iso.gost
wget --no-check-certificate https://dl.astralinux.ru/astra/stable/smolensk/security-updates/1.6/$iso_update/$iso_update$iso_ext.gost
g_summ=(`cat $iso_update$iso_ext".gost"`)
echo -e "\n\n\033[33;5;1m Проверка файла обновления на целостность\033[0m\n\n"
g_s=(`gostsum -d $path_update$iso_update$iso_ext`)
else
cd $path_update
echo -e "\n\n\033[33;5;1m Проверка файла обновления на целостность\033[0m\n\n"
g_s=(`gostsum -d $path_update$iso_update$iso_ext`)
...
if [[ ${g_summ[0]} != ${g_s[0]} ]]
then
wget --no-check-certificate -c https://dl.astralinux.ru/astra/stable/smolensk/security-updates/1.6/$iso_update/$iso_update$iso_ext
echo -e "\n\n\033[33;5;1m Проверка файла обновления на целостность после обновления\033[0m\n\n"
g_s=(`gostsum -d $path_update$iso_update$iso_ext`)
fi
fi
sudo -s <<EOF
...
echo -e "\033[41;30;5m ОБНОВЛЕНИЕ НЕВОЗМОЖНО. НЕ СОВПАДАЕТ КОНТРОЛЬНАЯ СУММА. УДАЛИТЕ ОБРАЗ ДИСКА ОБНОВЛЕНИЯ И ЗАПУСТИТЕ ОБНОВЛЕНИЕ ЗАНОВО.\033[0m\n\n"
echo -e "\033[31m Повторите процедуру обновления \033[0m"
exit
exit
fi
EOF
